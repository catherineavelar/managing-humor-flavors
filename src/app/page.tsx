...
export default function Home() {
  ...
  const [duplicateSource, setDuplicateSource] = useState<Flavor | null>(null)
  ...
  function openDuplicateModal(flavor: Flavor) {
    setDuplicateSource(flavor)
    setDuplicateName(`${flavor.description} (Copy)`)
  }

  async function confirmDuplicate() {
    if (!duplicateSource || !duplicateName.trim()) return
    setDuplicating(true)
    const newSlugVal = duplicateName.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' + Date.now()
    const { data: newFlavor, error } = await supabase
        .from('humor_flavors')
        .insert([{ description: duplicateName.trim(), slug: newSlugVal }])
        .select()
        .single()
    if (error || !newFlavor) { setDuplicating(false); return }
    const { data: sourceSteps } = await supabase
        .from('humor_flavor_steps')
        .select('*')
        .eq('humor_flavor_id', duplicateSource.id)
        .order('order_by')
  ...
  return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 flex flex-col">

        {/* Duplicate Modal */}
        {duplicateSource && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
              <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-7 w-full max-w-md mx-4">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Duplicate Flavor</h3>
                <p className="text-sm text-gray-400 mb-5">
                  Duplicating <span className="font-medium text-gray-700 dark:text-gray-300">"{duplicateSource.description}"</span> and all its steps. Give the duplicate a unique name.
                </p>
                <label className={labelCls}>New flavor name *</label>
                <input
                    autoFocus
                    type="text"
                    value={duplicateName}
                    ...
                    className={`mt-1 mb-5 ${inputCls}`}
                    placeholder="e.g. GenZ Speech v2"
                />
                <div className="flex gap-2">
                  <button onClick={confirmDuplicate} disabled={duplicating || !duplicateName.trim()} className="text-sm bg-gray-900 dark:bg-white dark:text-gray-900 text-white px-5 py-2 rounded-lg hover:bg-gray-700 dark:hover:bg-gray-100 disabled:opacity-50 transition font-medium">
                    {duplicating ? 'Duplicating...' : 'Duplicate'}
                  </button>
                  <button onClick={() => { setDuplicateSource(null); setDuplicateName('') }} className="text-sm border border-gray-200 dark:border-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition text-gray-600 dark:text-gray-400">
                    Cancel
                  </button>
                </div>
              </div>
        ...
        {/* ── TOP NAV ── */}
        <nav className="border-b border-gray-200 dark:border-gray-800 px-8 bg-white dark:bg-gray-900 flex items-center justify-between h-14 shrink-0">

          {/* Left: logo + name */}
          <div className="flex items-center gap-3">
            <img src="/logo3.png" alt="Flavor Manager" className="h-8 w-auto" />
            <span className="text-base font-bold text-gray-900 dark:text-white tracking-tight">Flavor Manager</span>
          </div>
          ...
          <div className="flex items-center gap-1">
            {([
              ['welcome',    '👋 Welcome'],
              ['flavors',    '🌶️ Explore Flavors'],
              ['new-flavor', '✨ Create a Flavor'],
              ['captions',   '💬 Captions Created'],
        ...
        <div className="flex-1 overflow-y-auto flex flex-col items-center">
          ...
          {/* WELCOME */}
          {activeTab === 'welcome' && (
              <div className="py-16 px-8 w-full max-w-3xl">
                <h2 className="text-5xl font-bold mb-4" style={{ color: '#a78bfa' }}>Train your humor style.</h2>
                <p className="text-gray-500 dark:text-gray-400 text-base mb-14 leading-relaxed max-w-xl">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <button onClick={() => setActiveTab('flavors')} className="border border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-left transition-all duration-200 bg-gradient-to-br from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 hover:-translate-y-1 hover:shadow-md hover:border-purple-200 dark:hover:border-purple-800">
                    <p className="text-3xl mb-4">🌶️</p>
                    <p className="font-semibold text-gray-900 dark:text-white mb-2">Explore Flavors</p>
                    <p className="text-xs text-gray-400 leading-relaxed">Browse, edit, and manage your existing prompt chains.</p>
                  </button>
                  <button onClick={() => setActiveTab('new-flavor')} className="border border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-left transition-all duration-200 bg-gradient-to-br from-white to-purple-50 dark:from-gray-900 dark:to-purple-900/20 hover:-translate-y-1 hover:shadow-md hover:border-purple-200 dark:hover:border-purple-800">
                    <p className="text-3xl mb-4">✨</p>
                    <p className="font-semibold text-gray-900 dark:text-white mb-2">Create a Flavor</p>
                    <p className="text-xs text-gray-400 leading-relaxed">Build a new prompt chain from scratch and add steps to it.</p>
                  </button>
                  <button onClick={() => setActiveTab('captions')} className="border border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-left transition-all duration-200 bg-gradient-to-br from-white to-blue-50 dark:from-gray-900 dark:to-blue-900/20 hover:-translate-y-1 hover:shadow-md hover:border-purple-200 dark:hover:border-purple-800">
                    <p className="text-3xl mb-4">💬</p>
                    <p className="font-semibold text-gray-900 dark:text-white mb-2">Captions Created by Our Flavors</p>
                    <p className="text-xs text-gray-400 leading-relaxed">Read all captions generated by each flavor.</p>
                  </button>
                  <div className="border border-gray-100 dark:border-gray-800 rounded-2xl p-8 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800/50">
                    <p className="text-3xl mb-4">💡</p>
                    <p className="font-semibold text-gray-900 dark:text-white mb-3">How it works</p>
                    <ol className="text-xs text-gray-400 space-y-2 leading-relaxed">
              ...
              <div className="p-8">
                <div className="flex items-center gap-3 mb-6 max-w-3xl">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 shrink-0">
                    <span>Sort by</span>
                    <select value={sortBy} onChange={e => setSortBy(e.target.value as 'recent' | 'az' | 'za')} className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-300">
                          ...
                          .map(flavor => (
                              <div key={flavor.id} className="flex items-center justify-between py-4 gap-4">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                  <button onClick={() => openDuplicateModal(flavor)} className="text-xs font-medium text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-800 px-3 py-1.5 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition">Duplicate</button>
                                  <button onClick={() => { setSelectedFlavor(flavor); setEditDesc(flavor.description); setEditSlug(flavor.slug); setEditingFlavor(true) }} className="text-xs font-medium text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition">Edit</button>
                                  <button onClick={() => { setSelectedFlavor(flavor); setDeletingFlavor(true) }} className="text-xs font-medium text-red-400 hover:text-red-600 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition">Delete</button>
                                </div>
                              </div>
                          ))
                      }
          ...
          {activeTab === 'flavors' && selectedFlavor && (
              <div className="p-8 max-w-3xl">
                <button onClick={() => setSelectedFlavor(null)} className="text-sm text-gray-400 hover:text-gray-700 mb-6 flex items-center gap-1 transition">← Back to All Flavors</button>
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-5 mb-6 bg-white dark:bg-gray-900">
                  {editingFlavor ? (
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex gap-2">
                          <button onClick={saveFlavor} disabled={savingFlavor} className="text-sm bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-700 disabled:opacity-50 transition">{savingFlavor ? 'Saving...' : 'Save'}</button>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => { setEditDesc(selectedFlavor.description); setEditSlug(selectedFlavor.slug); setEditingFlavor(true) }} className="text-sm text-gray-600 hover:text-gray-900 border border-gray-200 px-3 py-1.5 rounded-lg transition">Edit</button>
                          {deletingFlavor ? (
                              <div className="flex gap-2">
                              ...
                              <button onClick={() => setDeletingFlavor(true)} className="text-sm text-gray-600 hover:text-red-500 border border-gray-200 px-3 py-1.5 rounded-lg transition">Delete</button>
                          )}
                        </div>
                      </div>
                  )}
                ...
                <div className="flex gap-1 border-b border-gray-200 dark:border-gray-700 mb-6">
                  <button onClick={() => setFlavorTab('steps')} className={`text-sm px-4 py-2 border-b-2 transition -mb-px ${flavorTab === 'steps' ? 'border-purple-500 text-purple-700 font-medium' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700'}`}>Steps — {steps.length}</button>
                  <button onClick={() => setFlavorTab('reorder')} className={`text-sm px-4 py-2 border-b-2 transition -mb-px ${flavorTab === 'reorder' ? 'border-purple-500 text-purple-700 font-medium' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700'}`}>Reorder Steps</button>
                  <button onClick={() => setFlavorTab('test')} className={`text-sm px-4 py-2 border-b-2 transition -mb-px ${flavorTab === 'test' ? 'border-purple-500 text-purple-700 font-medium' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700'}`}>Test Captions</button>
                </div>
                          ...
                          <>
                                  ...
                                  {editingStep === step.id ? (
                                      <div className="p-5 flex flex-col gap-3 bg-gray-50 dark:bg-gray-800">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                          <div><label className={labelCls}>Temperature</label><input type="number" step="0.1" min="0" max="2" value={editStepForm.llm_temperature || 0.7} onChange={e => setEditStepForm({ ...editStepForm, llm_temperature: parseFloat(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                          <div><label className={labelCls}>Input Type ID</label><input type="number" value={editStepForm.llm_input_type_id || 1} onChange={e => setEditStepForm({ ...editStepForm, llm_input_type_id: parseInt(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                          <div><label className={labelCls}>Output Type ID</label><input type="number" value={editStepForm.llm_output_type_id || 2} onChange={e => setEditStepForm({ ...editStepForm, llm_output_type_id: parseInt(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                          <div><label className={labelCls}>Model ID</label><input type="number" value={editStepForm.llm_model_id || 1} onChange={e => setEditStepForm({ ...editStepForm, llm_model_id: parseInt(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                        </div>
                                        <button onClick={saveStep} disabled={savingStep} className="self-start text-sm bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-700 disabled:opacity-50 transition">{savingStep ? 'Saving...' : 'Save Step'}</button>
                                      </div>
                                  ) : (
                            ...
                            {showNewStep && (
                                <div className="border border-purple-200 dark:border-purple-800 rounded-xl p-5 flex flex-col gap-3 bg-purple-50 dark:bg-purple-900/20">
                                  ...
                                  <div><label className={labelCls}>Description</label><input value={newStep.description} onChange={e => setNewStep({ ...newStep, description: e.target.value })} className={`mt-1 ${inputCls}`} placeholder="Optional" /></div>
                                  <div><label className={labelCls}>System Prompt *</label><textarea value={newStep.llm_system_prompt} onChange={e => setNewStep({ ...newStep, llm_system_prompt: e.target.value })} rows={3} className={`mt-1 ${inputCls} resize-none`} placeholder="You are an eagle-eyed image describer..." /></div>
                                  <div><label className={labelCls}>User Prompt *</label><textarea value={newStep.llm_user_prompt} onChange={e => setNewStep({ ...newStep, llm_user_prompt: e.target.value })} rows={3} className={`mt-1 ${inputCls} resize-none`} placeholder="Describe this image in detail..." /></div>
                                  <div className="grid grid-cols-2 gap-3">
                                    <div><label className={labelCls}>Temperature</label><input type="number" step="0.1" min="0" max="2" value={newStep.llm_temperature} onChange={e => setNewStep({ ...newStep, llm_temperature: parseFloat(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                    <div><label className={labelCls}>Input Type ID</label><input type="number" value={newStep.llm_input_type_id} onChange={e => setNewStep({ ...newStep, llm_input_type_id: parseInt(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                    <div><label className={labelCls}>Output Type ID</label><input type="number" value={newStep.llm_output_type_id} onChange={e => setNewStep({ ...newStep, llm_output_type_id: parseInt(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                    <div><label className={labelCls}>Model ID</label><input type="number" value={newStep.llm_model_id} onChange={e => setNewStep({ ...newStep, llm_model_id: parseInt(e.target.value) })} className={`mt-1 ${inputCls}`} /></div>
                                  </div>
                                  <button onClick={addStep} disabled={savingStep || !newStep.llm_system_prompt.trim() || !newStep.llm_user_prompt.trim()} className="self-start text-sm bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-700 disabled:opacity-50 transition">{savingStep ? 'Adding...' : 'Add Step'}</button>
                                </div>
                            )}
                            {!showNewStep && (
                                <button onClick={() => setShowNewStep(true)} className="w-full border-2 border-dashed border-gray-200 hover:border-purple-300 text-gray-400 hover:text-purple-500 py-4 rounded-xl transition text-sm font-medium">+ Add Step</button>
                            )}
                          </>
                      )}
                    </div>
                ...
                {flavorTab === 'reorder' && (
                    <div className="flex flex-col gap-4 max-w-xl">
                      ...
                      {reorderSteps.length === 0 && <p className="text-sm text-gray-400">No steps to reorder yet.</p>}
                      <div className="flex flex-col gap-2">
                        {reorderSteps.map((step, idx) => (
                            <div key={step.id} draggable onDragStart={() => setDragIdx(idx)}
                                 onDragOver={e => { e.preventDefault(); if (dragIdx === null || dragIdx === idx) return; const arr = [...reorderSteps]; const dragged = arr.splice(dragIdx, 1)[0]; arr.splice(idx, 0, dragged); setReorderSteps(arr); setDragIdx(idx) }}
                                 onDragEnd={() => setDragIdx(null)}
                                 className={`border rounded-xl px-4 py-3 flex items-center gap-4 cursor-grab active:cursor-grabbing transition select-none ${dragIdx === idx ? 'border-purple-400 bg-purple-50 dark:bg-purple-900/20 shadow-lg scale-[1.02]' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:border-gray-300 dark:hover:border-gray-600'}`}>
                              ...
                              <div className="flex-1 min-w-0">
                                {step.description && <p className="text-xs font-semibold text-gray-400 uppercase mb-0.5">{step.description}</p>}
                      ...
                      {reorderSteps.length > 0 && (
                          <div className="flex items-center gap-3">
                            <button onClick={saveReorder} disabled={savingReorder} className="text-sm bg-gray-900 dark:bg-white dark:text-gray-900 text-white px-5 py-2 rounded-lg hover:bg-gray-700 dark:hover:bg-gray-100 disabled:opacity-50 transition font-medium">{savingReorder ? 'Saving...' : 'Save Order'}</button>
                            {reorderSuccess && <span className="text-sm text-green-600 dark:text-green-400">✓ Order saved!</span>}
                          </div>
                      )}
                    </div>
                )}
                ...
                {flavorTab === 'test' && (
                    <div className="flex flex-col gap-5 max-w-xl">
                      ...
                      {steps.length === 0 && <div className="bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-lg px-4 py-3">⚠️ This flavor has no steps yet. Add steps before testing.</div>}
                      <div>
                        <label className={labelCls}>Upload Image</label>
                        <div onClick={() => fileInputRef.current?.click()} className="mt-2 border-2 border-dashed border-gray-200 hover:border-purple-300 rounded-xl p-6 cursor-pointer transition flex flex-col items-center gap-2">
                          {imagePreview ? <img src={imagePreview} alt="Preview" className="max-h-48 rounded-lg object-contain" /> : <><span className="text-2xl">📷</span><p className="text-sm text-gray-400">Click to upload an image</p><p className="text-xs text-gray-300">JPEG, PNG, WebP, GIF, HEIC</p></>}
                        </div>
                        ...
                        {imageFile && <p className="text-xs text-gray-400 mt-1">{imageFile.name} · {(imageFile.size / 1024).toFixed(0)} KB</p>}
                      </div>
                      <button onClick={generateCaptions} disabled={!imageFile || generating || steps.length === 0} className="self-start bg-gray-900 hover:bg-gray-700 disabled:opacity-40 text-white font-medium px-5 py-2.5 rounded-lg transition text-sm flex items-center gap-2">
                        {generating ? <><span className="animate-spin inline-block">⚙️</span> Generating...</> : <>🚀 Generate Captions</>}
                      </button>
                      {captionError && <div className="bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-3">Error: {captionError}</div>}
                      {captions.length > 0 && (
                          <div>
                ...
                {!captionsFlavor ? (
                    <>
                      ...
                      <div className="flex items-center gap-3 mb-6 max-w-2xl">
                        <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                          <button onClick={() => setCaptionsFilter('all')} className={`text-xs px-3 py-1.5 rounded-md transition font-medium ${captionsFilter === 'all' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>All Flavors</button>
                          <button onClick={() => setCaptionsFilter('has-captions')} className={`text-xs px-3 py-1.5 rounded-md transition font-medium ${captionsFilter === 'has-captions' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>Has Captions</button>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                          <span>Sort by</span>
                          <select value={captionsSort} onChange={e => setCaptionsSort(e.target.value as 'most' | 'az' | 'za')} className="border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-1.5 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-300">
                      ...
                      {loading ? <p className="text-gray-400 text-sm">Loading...</p> : (
                          <div className="flex flex-col gap-3 max-w-2xl">
                            {flavors.length === 0 && <p className="text-gray-400 text-sm">No flavors yet.</p>}
                            {flavors
                                .filter(f => captionsFilter === 'has-captions' ? (captionCounts[f.id] ?? 0) > 0 : true)
                                .sort((a, b) => captionsSort === 'most' ? (captionCounts[b.id] ?? 0) - (captionCounts[a.id] ?? 0) : captionsSort === 'az' ? (a.description ?? '').localeCompare(b.description ?? '') : (b.description ?? '').localeCompare(a.description ?? ''))
                                .map(flavor => (
                                    <div key={flavor.id} onClick={() => { setCaptionsFlavor(flavor); fetchFlavorCaptions(flavor.id) }} className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 cursor-pointer hover:border-purple-300 dark:hover:border-purple-700 transition bg-white dark:bg-gray-900">
                    ...
                    <div className="max-w-2xl">
                      <button onClick={() => { setCaptionsFlavor(null); setFlavorCaptions([]) }} className="text-sm text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 mb-6 flex items-center gap-1 transition">← Back to Captions</button>
                      ...
                      {!captionsLoading && !captionsError && flavorCaptions.length === 0 && (
                          <div className="border border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-8 text-center">
                            ...
                            <p className="text-gray-300 dark:text-gray-600 text-xs mt-1">Go to Explore Flavors → Test Captions to generate some.</p>
                          </div>
                      )}
              ...
              <div className="p-8 max-w-lg">
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-6 bg-white dark:bg-gray-900 flex flex-col gap-5">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={createFlavor} disabled={creating} className="text-sm bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-lg px-5 py-2.5 hover:bg-gray-700 dark:hover:bg-gray-100 disabled:opacity-50 transition font-medium">{creating ? 'Creating...' : 'Create Flavor'}</button>
                    <button onClick={() => { setNewDesc(''); setNewSlug(''); setCreateError(''); setCreateSuccess('') }} className="text-sm border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 rounded-lg px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 transition">Clear</button>
                  </div>
                </div>
              </div>
          )}
